...
export function UserProfileDetails({ user }: UserProfileDetailsProps) {
  ...
  return (
    <Card
      className={cn(
        "@container relative max-w-full h-full flex flex-col min-h-64",
        "rounded-md border overflow-hidden shadow-none",
        likesPizza && "border-orange-300 dark:border-orange-800",
      )}
    >
      <div className="relative w-full h-32 @lg:h-48 rounded-t-inherit">
      ...
      <CardContent className="w-full grid grid-cols-2 p-4 pt-12 @lg:pt-16 bg-white dark:bg-zinc-950">
        {likesPizza && (
        ...
        <h1 className="row-start-1 col-start-1 font-semibold text-xl @md:text-2xl">
          {name}
        </h1>
        <span className="row-start-2 col-start-1 text-muted-foreground">
          {role}
        </span>
        <p className="mt-2 text-muted-foreground col-span-full @lg:max-w-md">
          {description}
        </p>
      </CardContent>
    </Card>
